...
## 서론
컴퓨터를 이용한 정보 처리가 점차 늘어나고 있고, 동시에 과거의 여러가지 기록물을 디지털화 하는 과정에서 효율성을 추구하기 위해 OCR 등의 방식을 이용하고 있다. OCR 기술은 현재까지 20년 넘게 사용되었으며, 문서나 이미지만을 인식하던 기술에서 일반적인 사진이나 동영상 속으 문자 까지도 인식할 수 있는 기술로 발전하고 있다. OCR은 이미지 및 문서 처리 업무를 자동화하여 효율 극대화 및 비용 절감에 효과적이고, 특히 모바일 사용이 대중화되며 기업이 처리해야 할 스캔 이미지가 모바일 이미지로 바뀌고 있어 그 필요성이 점차 높아지고 있다.
## 연구 동기 및 목적
여러 텍스트 인식 프로그램들을 보고, 교과서나 학교 교과 수업 유인물 등을 디지털 문서화한다면 편리할 것 같아서 이미지를 통해 한글을 디지털화하는 프로젝트를 하고자 하였다. 또한, OCR 기술을 딥러닝과 접목하여 기존의 OCR 대비 높은 효율성을 내고자 하였다. 기존까지의 OCR의 구조는 영상의 이진화, 문자 영역의 검출, 라인 또는 단어 단위의 추출, 단어 단위 이미지를 텍스트로 변환하기 위해 문자를 하나씩 인식하고 결합하는 구조를 이용한다. 하지만, 딥 러닝을 이용해 OCR을 구현한다면 기존과는 다르게 품질이 높고 상대적으로 적은 양의 데이터로 충분한 정확성을 얻을 수 있다.
## 이론적 배경
